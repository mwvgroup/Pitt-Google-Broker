# Pub/Sub configuration for ZTF.

resource "google_pubsub_topic" "alerts" {
  name = "ztf-alerts${local.dashed_test_suffix}"
}
resource "google_pubsub_topic" "bq_import" {
  name = "ztf-bigquery-import${local.dashed_test_suffix}"
}
resource "google_pubsub_topic" "bq_deadletter" {
  name = "ztf-bigquery-import-deadletter${local.dashed_test_suffix}"
}

resource "google_pubsub_subscription" "alerts-reservoir" {
  name = "ztf-alerts-reservoir${local.dashed_test_suffix}"
