...
def load_test_alert(survey):
    if survey == "lsst":
        path = "tests/lsst.json"
        with open(path, "r") as f:
            data = json.load(f)
        alert = Alert.from_dict(data, schema_name="default")
    else:
        alert = Alert.from_path(test_alert_path, schema_name="ztf")

    return alert


def test_lsst_create_dataframe():
    """_create_dataframe should return a DataFrame with expected columns"""
    alert_lite = load_test_alert(survey="lsst")
...
def test_lsst_format_for_upsilon():
...
def test_ztf_create_dataframe():
    """_create_dataframe should return a DataFrame with expected columns"""
    alert_lite = load_test_alert(survey="ztf")
    alert_lite_dict = alert_lite.dict["alert_lite"]
    df = _create_dataframe(alert_lite_dict)
...
def test_ztf_format_for_upsilon():
    """_format_for_classifier should produce DataFrame with correct SuperNNova columns"""
    alert = load_test_alert()
    df = _format_for_classifier(alert)
