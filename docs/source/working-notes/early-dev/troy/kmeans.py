...
def load_clust_plot(
    f="upsilon_features.dat", fcsv="asassn-catalog.csv", cfeat=None, normfeats=True
):
    ...
    # run Kmeans
    predics, predics_test, dists, dists_test = get_kclusts(
        db, dflow, normfeats=normfeats, color=cfeat
    )

    # make plots
    make_plots(
        db, dflow, predics, predics_test, cfeat=cfeat, distances=(dists, dists_test)
    )
...
def make_plots(db, dflow, predics, predics_test, cfeat=None, distances=None):
    ...
    # confusion matrix
    for d, p, lbl in zip(
        [db, dflow], [predics, predics_test], ["predics", "predics_test"]
    ):
        clss = d.groupby("Type").mean().intType.astype("int").sort_values().index
        plot_confusion_matrix(
            d.intType, p, clss, normalize=True, title="Confusion Matrix"
        )
    ...
    if distances is not None:
        ...
        ax = plt.gca()
        for d, lbl in zip(
            [distances[0], distances[1]], ["training set distance", "test set distance"]
        ):
