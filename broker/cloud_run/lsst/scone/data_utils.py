...
def stratified_split(dataset, train_proportion, types, include_test_set, class_balance):
    by_type_data_lists = {
        sn_type: dataset.filter(lambda image, label, *_: label["label"] == sn_type)
    ...
    for sn_type, data in by_type_data_lists.items():
        ...
        current_val = (
            current_test_val if not include_test_set else current_test_val.take(num_in_val)
        )
    ...
    if class_balance:
        full_dataset_size = min_amount * len(
            by_type_data_lists.keys()
        )  # full dataset size = heatmaps per type * num types
    else:
...
def get_dataset_makeup(dataset):
    ...
    for i, elem in enumerate(dataset):
        sntype = elem[1].numpy()
        relative_abundance[sntype] = (
            1 if sntype not in relative_abundance else relative_abundance[sntype] + 1
        )
